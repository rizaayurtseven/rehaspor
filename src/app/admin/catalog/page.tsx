...
import { BookOpen, CheckCircle2, Clock3, ExternalLink, FileText, Languages, Save, Loader2 } from "lucide-react";
...
export default function AdminCatalogPage() {
  ...
  return (
    <>
      <AdminHeader title="E-Katalog Yönetimi" description="Türkçe ve İngilizce PDF katalog dosyalarını kontrol edin ve yayın durumlarını yönetin." />
        ...
        <section className="mb-6 overflow-hidden rounded-2xl bg-brand-navy p-6 text-white shadow-card sm:flex sm:items-center sm:justify-between sm:gap-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Yüklenen dosyalar, public sitedeki E-Katalog sayfasında görüntüleme ve indirme bağlantıları için kullanılacaktır.</p>
            </div>
          </div>
              ...
              <article key={catalog.id} className="overflow-hidden rounded-2xl border border-brand-line bg-white shadow-sm">
                <div className="flex items-start justify-between gap-4 border-b border-brand-line p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <span className="inline-flex rounded-full bg-brand-navy px-2 py-0.5 text-[10px] font-black tracking-wider text-white">{catalog.language}</span>
                      <h2 className="mt-1.5 text-lg font-black text-brand-navy">{catalog.title}</h2>
                    </div>
                  </div>
                  <span className={catalog.status === "PUBLISHED" ? "rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-black text-emerald-700" : "rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-black text-amber-700"}>
                ...
                <div className="p-5">
                  ...
                  <div className="mt-5">
                    <ImageUploaderPlaceholder label={`${catalog.language} katalog PDF'i`} helper="PDF · maksimum 25 MB" accept="application/pdf" />
                  </div>
                  <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                    ...
                    >
                      <ExternalLink size={16} /> Mevcut PDF
                    </a>
                  </div>
                </div>
              </article>
